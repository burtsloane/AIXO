...
#include <cmath> // Include if needed

// Inherits only from PWRJ_MultiSelectJunction now
class SS_Rudder : public PWRJ_MultiSelectJunction
{
protected:
...
public:
    SS_Rudder(const FString& Name, ASubmarineState* InSubState, float InX, float InY, float InW=150, float InH=24)
	...
	virtual FString GetTypeString() const override { return TEXT("SS_Rudder"); }
    ...
    virtual TArray<FString> QueryEntireState() const override
    {
        ...
        // Append state from power part (base class)
    ...
    virtual void Tick(float DeltaTime) override
    {
        ...
        // This class (SS_Rudder) might need to read the CurrentAngle from ActuatorPart
        // if other systems (like SubmarineState) need it directly from here.
        // Example: 
        SubState->RudderAngle = ActuatorPart.GetCurrentAngle(); 
    }
	...
	virtual void RenderUnderlay(RenderingContext& Context) override
	{
		...
		a.X = X + W/2;				a.Y = Y + H - 2;
        b = a;
		b.X -= R*FMath::Sin(-GetCurrentAngle() * 1.0f);
		b.Y += R*FMath::Cos(-GetCurrentAngle() * 1.0f);
		...
		for (int i=-7; i<7; i++) {
			...
			FVector2D a1 = a;
			a1.X -= R*FMath::Sin(ang);
			a1.Y += R*FMath::Cos(ang);
			b = a;
			b.X -= R2*FMath::Sin(ang);
			b.Y += R2*FMath::Cos(ang);
			Context.DrawLine(a1, b, FLinearColor::Black, ((i&1)==0)?2.0f:1.0f);
		}
	}

    // Expose ActuatorPart's TargetAngle if needed by PID_RudderHeading
    // Note: PID_RudderHeading.h currently sets Rudder->TargetAngle directly.
    // This needs changing if TargetAngle in ICH_Actuator is protected.
    // Provide a setter method as the preferred approach.
    ...
    friend class PID_RudderHeading; // Still needed if PID needs access
};
