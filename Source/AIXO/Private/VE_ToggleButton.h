 ...
 */
class VE_ToggleButton : public IVisualElement
{
protected:
...
public:
    VE_ToggleButton(ICH_PowerJunction* InOwner, 
...
//if (InTextOn.Len() != InTextOff.Len()) UE_LOG(LogTemp, Warning, TEXT("VE_ToggleButton: string length mismatch '%s' '%s'"), *InTextOn, *InTextOff);
    }
			...
			case TouchEvent::EType::Up:
				{
					if (isInside)
					{
						...
						const FString& CommandFragment = bIsCurrentlyOn ? CommandToSendOff : CommandToSendOn;
